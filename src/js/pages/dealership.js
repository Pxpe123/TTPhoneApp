...
function CreateDynamicBoxes() {
  ...
  Object.keys(response).forEach(function (category) {
    ...
    var adropdownId =
      "adropdown" + (Object.keys(response).indexOf(category) + 1);
    ...
    document
      .getElementById("dealershipInnerPage")
...
function SetDealershiptitle() {
  const DealershipTitle = document.getElementById("DealershipTitle");
  var timestamp = response.timestamp;
  var date = new Date(timestamp);
  var formattedDateTime = date.toLocaleString();
  var dealershipText = `Dealership (${formattedDateTime})`;
  DealershipTitle.innerText = dealershipText;
}
...
async function requestUpdate() {
  var userdataURL = "http://141.145.202.202:3001/RequestDealership";
  const fetchOptions = {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
  ...
  try {
    const updateResponse = await fetch(userdataURL, fetchOptions);
    const responseData = await updateResponse.json();
    console.log(responseData);

    if (responseData.status === "Data Updated") {
      setupDealershipPage();
    }
  } catch (error) {
